...
import math
from engine import Engine2D, Position2D, clamp

class Window(tk.Tk, Engine2D):
    def __init__(self):
        # Initialize tk and Engine2D
        tk.Tk.__init__(self, "Engine2D - v2")
        Engine2D.__init__(self)

        self.geometry = f"{self.winfo_screenwidth()}x{self.winfo_screenheight()}"

        self.center = Position2D(0, 0)
        self.zoom = 1
        self.canvas = tk.Canvas(self, width=self.winfo_screenwidth(), height=self.winfo_screenheight())
        self.canvas.pack()

        self.grid()
        self.bind('<MouseWheel>', self.updateZoom) 
        self.mainloop() 

    def updateZoom(self, event: tk.Event):
    ...
    def grid(self):
        ...
        log = abs(math.ceil(math.log2(self.zoom)))

        ylines = log % 20 + 10
        xlines = math.floor(w / (h/ylines))

        for i in range(ylines + 1):
            y = h*i/ylines
            grd.add(self.canvas.create_line(0, y, w, y, fill="red", tags="grid"))
        for i in range(xlines + 1):
            x = w*i/xlines
            grd.add(self.canvas.create_line(x, 0, x, h, fill="red", tags="grid"))
...
#     vertLineSetNew = []
#     horLineSetNew = []
#     for x in vertLineSet:
#         x1, y1 = virToScreen(x=x, y=ohoek.y)
#         x2, y2 = virToScreen(x=x, y=lhoek.y)
